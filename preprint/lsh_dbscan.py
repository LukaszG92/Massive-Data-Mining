...
class LSHApproximateDBSCAN:
    def __init__(self, eps: float, min_pts: int, c: float = 1.5, delta: float = 0.5, center_ratio: float = 1.0, ratio_offset: int = 0, verbose: bool = True):
        self.eps = eps
    ...
    def _construct_lsh_family(self, X: np.ndarray) -> Dict:
        ...
        p2 = 1 - 2*norm.cdf(-w/self.c) - (2/(math.sqrt(2*math.pi) * w/self.c)) * (1-math.exp(-(w**2 / (2*self.c**2))))
